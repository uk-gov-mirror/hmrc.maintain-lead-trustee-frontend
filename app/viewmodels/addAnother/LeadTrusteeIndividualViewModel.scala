...
final case class LeadTrusteeIndividualViewModel(isLead : Boolean,
                                                name : Option[String],
                                                `type` : Option[IndividualOrBusiness],
                                                status : TrusteeStatus)

object LeadTrusteeIndividualViewModel {
  ...
  val nameReads : Reads[Option[String]] =
    (__ \ "name").read[Name].map(_.toString.toOption) orElse
      (__ \ "name").readNullable[String]

  implicit lazy val reads : Reads[LeadTrusteeIndividualViewModel] = (
      nameReads and
      (__ \ "status").readWithDefault[TrusteeStatus](InProgress)
    )(
    (name, status) => {
      LeadTrusteeIndividualViewModel(true, name, Some(IndividualOrBusiness.Individual), status)
    }
  )
