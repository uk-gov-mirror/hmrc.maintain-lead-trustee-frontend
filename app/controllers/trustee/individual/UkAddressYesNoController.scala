...
import pages.trustee.individual.UkAddressYesNoPage
...
class UkAddressYesNoController @Inject()(
  ...
  def onPageLoad(mode: Mode, index: Int): Action[AnyContent] = standardActionSets.IdentifiedUserWithData.andThen(nameAction(index)) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(UkAddressYesNoPage(index)) match {
        case None => form
  ...
  def onSubmit(mode: Mode, index: Int): Action[AnyContent] = standardActionSets.IdentifiedUserWithData.andThen(nameAction(index)).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(UkAddressYesNoPage(index), value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(UkAddressYesNoPage(index), mode, updatedAnswers))
      )
  }
}
