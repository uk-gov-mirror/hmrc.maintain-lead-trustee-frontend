...
import pages.trustee.organisation.{CountryOfResidenceYesNoPage, NamePage}
import play.api.data.Form
import play.api.inject.bind
import play.api.mvc.Call
import play.api.test.FakeRequest
import play.api.test.Helpers._
import views.html.trustee.organisation.CountryOfResidenceYesNoView
...
class CountryOfResidenceYesNoControllerSpec extends SpecBase with MockitoSugar {
  ...
  val form: Form[Boolean] = formProvider.withPrefix("trustee.organisation.countryOfResidenceYesNo")
  val trusteeName = "FirstName LastName"

  override val emptyUserAnswers = UserAnswers("id", "UTRUTRUTR", LocalDate.now())
    .set(NamePage, trusteeName).success.value

  lazy val countryOfResidenceYesNo: String = routes.CountryOfResidenceYesNoController.onPageLoad(NormalMode).url
