...
import models.{IdCard, IdentificationDetailOptions, Name, NonUkAddress, Passport, UkAddress}
  ...
  "LeadTrusteeOrganisationPrintHelper" must {
    ...
    "generate lead trustee organisation section for all possible data" in {
      ...
      val userAnswers = emptyUserAnswers
        ...
        .set(IdentificationDetailOptionsPage, IdentificationDetailOptions.Passport).success.value
        .set(IdCardDetailsPage, IdCard("DE", "number", LocalDate.of(1996, 2, 3))).success.value
        .set(PassportDetailsPage, Passport("DE", "number", LocalDate.of(1996, 2, 3))).success.value
      ...
      result mustBe AnswerSection(
        ...
        rows = Seq(
          ...
          AnswerRow(label = Html(messages("leadtrustee.individual.identificationDetailOptions.checkYourAnswersLabel", name.displayName)), answer = Html("Passport"), changeUrl = controllers.leadtrustee.individual.routes.IdentificationDetailOptionsController.onPageLoad().url),
          AnswerRow(label = Html(messages("leadtrustee.individual.idCardDetails.checkYourAnswersLabel", name.displayName)), answer = Html("Germany<br />number<br />3 February 1996"), changeUrl = controllers.leadtrustee.individual.routes.IdCardDetailsController.onPageLoad().url),
          AnswerRow(label = Html(messages("leadtrustee.individual.passportDetails.checkYourAnswersLabel", name.displayName)), answer = Html("Germany<br />number<br />3 February 1996"), changeUrl = controllers.leadtrustee.individual.routes.PassportDetailsController.onPageLoad().url),
