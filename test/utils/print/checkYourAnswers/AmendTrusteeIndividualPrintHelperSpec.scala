    ...
    "generate individual trustee section for all possible data" in {
      ...
      result mustBe AnswerSection(
        ...
        rows = Seq(
          ...
          AnswerRow(label = Html(messages("trustee.individual.nationalInsuranceNumber.checkYourAnswersLabel", name.displayName)), answer = Html("AA 00 00 00 A"), changeUrl = controllers.trustee.amend.individual.routes.NationalInsuranceNumberYesNoController.onPageLoad().url),
